...
class OsmBuffer {
   ...
   public:
    ...
    int getOsmBufSize() { return osmBufSize; }

    int getOsmBufCap() { return osmCap; }
    ...
    int getNumZones() { return num_zones; }

    int getCurrentZone() { return current_zone; }

    int getPreviousBoundary() { return previous_boundary; }

    std::pair<_Key, _Value> *getOsmBuffer() { return osmBuffer; }

    std::pair<_Key, _Key> *getOsmZones() { return zones; }

    int getLastSortedZone() { return last_sorted_zone; }

    void setSortedBuffer(bool flag) { sorted_buffer = flag; }

    OsmBufferCounters getCounters() { return counters; }
   ...
   public:
    ...
    void updateZonemaps() {
        ...
        // since we have flushed elements (at least a since page/zone), this
        // would always be less than our zones capacity
        assert(num_required_zones < num_zones);

        // update zone maps is always done after the sorting process
        // so the first element and the last element in every zone would be our
        // min and max
        ...
        if (current_zone_element_count > 0 &&
            current_zone_element_count < num_per_zone) {
            bool flag = true;
   ...
   public:
    void merge(std::pair<_Key, _Value> array[], int const left, int const mid,
               int const right) {
    ...
    void sortOsmBuf() {
        ...
        if (out_of_order_elements != 0) {
            ...
            if ((out_of_order_elements < k_limit_1 &&
                 max_discrepancy < l_limit_1) ||
    ...
    void sortOsmBufNew() {
        ...
        if (out_of_order_elements != 0) {
            ...
            if (atleast_one_subsorted) {
                ...
                std::stable_sort(osmBuffer + start_index,
                                 osmBuffer + osmBufSize);

                // now, our entire buffer consists of sorted blocks
                // we simply need to call merge on pairs of those blocks until
                // we have one left

                // our sorted blocks are as follows:
                // 0...previous_boundary,
                // previous_boundary+1...subsorted_boundaries[0],
                // subsorted_boundaries[1]...subsorted[subsorted_size-1],
                ...
                while (num_blocks_to_merge > 1) {
                    mid_index =
                        (num_per_zone * (subsorted_boundaries[i] + 1)) - 1;
                    assert(i + 1 < subsorted_size);
                    end_index =
                        num_per_zone * (subsorted_boundaries[i + 1] + 1) - 1;
                ...
                int pre_sorted_end =
                    (num_per_zone * (previous_boundary + 1)) - 1;
    ...
    bool insertInOsmBuf(std::pair<_Key, _Value> element) {
        ...
        if (element.first >= last_element) {
            ...
            if (current_zone == 0 || last_sorted_zone == -1) {
                last_sorted_zone = -1;
            } else if (element.first < zones[last_sorted_zone]
                                           .second)  // was second; not as bug
            {
                ...
                if (z >= 0) {
                    ...
                    last_sorted_zone =
                        -1;  // basically z will return -1 so set to -1
                    // max_discrepancy = current_zone + 1; // since the element
                    // we had is smaller than smallest in buffer,
                    // max_discrepency is equal to cur. length of buffer
                }
            }
            ...
            if (last_sorted_zone >= previous_boundary) {
                ...
                if (last_sorted_zone == -1) {
                    ...
                    discrepancy = current_zone - last_sorted_zone;
                } else if (element.first >= zones[last_sorted_zone].second &&
                           element.first <= zones[previous_boundary].second) {
                    int z = searchZone(element.first, last_sorted_zone + 1,
                                       previous_boundary);
...
// add element to bloom filter
        ...
        auto duration_globalbf =
            std::chrono::duration_cast<std::chrono::nanoseconds>(
                stop_globalbf - start_globalbf);
        ...
        // if map is empty or zone does not exist, simply add min and max for
        // zone as element
        ...
        auto duration_sublevel =
            std::chrono::duration_cast<std::chrono::nanoseconds>(
                stop_sublevel - start_sublevel);
        ...
        // if current element is out of order, check if it is the first one in
        // this cycle
        ...
        // now we have to update the sortedness zonemaps for sorted and unsorted
        // sections if sorted_buffer is true, we can simply check current
        // element against sorted zonemap's min and max else we check against
        // unsorted
        if (sorted_buffer) {
            // if this is the first element in the buffer, initialize both min
            // and max to this key
            if (osmBufSize == 1) {
        ...
        auto duration_ins =
            std::chrono::duration_cast<std::chrono::nanoseconds>(stop_ins -
                                                                 start_ins);
   ...
   public:
    ...
    int interpolationSearchKey(_Key key) {
        ...
        while (low <= high && key >= osmBuffer[low].first &&
               key <= osmBuffer[high].first) {
            if (low == high) {
                if (osmBuffer[low].first == key) return low;

                return -1;
            }

            int pos = low + ((double)(high - low) /
                             (osmBuffer[high].first - osmBuffer[low].first) *
                             (key - osmBuffer[low].first));
   ...
   public:
    ...
    void get_qualifying_zones(int qualifying_zones[], int &num_qualified,
    ...
    // more helpers
   public:
    bool isAtLeastOneZoneSorted() { return last_sorted_zone >= 0; }

    bool isBufferSorted() { return sorted_buffer; }
    ...
    void performPostFlushProcedure() {
        ...
        // after update, current_zone tells us the currently operating zone
        // which would also be the last sorted one
        last_sorted_zone = current_zone - 1;
        out_of_order = false;
        ...
        // from subsequent inserts, last_sorted_zone will change but
        // previous_boundary remains the same
        previous_boundary = current_zone - 1;
    ...
    bool query(_Key key) {
        ...
        if (within_buf_range) {
            ...
            if (within_unsorted_section_range) {
                ...
                // check bloom filter to see if element can exist in unsorted
                ...
                if (result) {
                    ...
                    if (atleast_one_subsorted) {
                        // number of filled pages post last sort will be
                        // currentzone - last boundary
                        last_boundary =
                            subsorted_boundaries[subsorted_size - 1];
...
#ifdef OSMP
                    auto start_unsorted_sorting =
                        std::chrono::high_resolution_clock::now();
#endif
                    if (filled_percent > fill_threshold) {
                        // sort the pages from last boundary+1 to current
                        // boundary included
                        int start_index = (num_per_zone * (last_boundary + 1));
                        int end_index =
                            (num_per_zone * (current_boundary + 1)) -
                            1;  // -1 for starting from 0 index
                        assert(end_index < osmBufSize && end_index < osmCap);
                        std::sort(osmBuffer + start_index,
                                  osmBuffer + end_index + 1);
                        // update the zonemaps concerning last_boundary to
                        // current_boundary included
                        updateZonemaps(last_boundary + 1, current_boundary + 1);

                        atleast_one_subsorted = true;

                        // add the new boundary to subsorted_boundaries;
                        // remember it is the right side of the boundary
                        // (inclusive)
...
#ifdef OSMP
                    auto stop_unsorted_sorting =
                        std::chrono::high_resolution_clock::now();
                    auto duration_unsorted_sorting =
                        std::chrono::duration_cast<std::chrono::nanoseconds>(
                            stop_unsorted_sorting - start_unsorted_sorting);
                    unsorted_sorting_time += duration_unsorted_sorting.count();
#endif

                    // new modified procedure....
                    // first sequentially scan from last_boundary+1 till
                    // current_zone next, if subsorted_size > 0, then in a while
                    // loop do interpolation search on each subsorted section.
                    assert(last_boundary <= current_zone);

                    // sequentially scan zonemaps to see if a zone can contain
                    // the key
                    ...
                    int num_qualified = 0;
#ifdef OSMPROFILE
                    auto start_zone_scan =
                        std::chrono::high_resolution_clock::now();
#endif
                    // change start boundary from previous_boundary to
                    // last_boundary get_qualifying_zones(qualifying_zones,
                    // num_qualified, key, last_boundary + 1);
#ifdef OSMPROFILE
                    auto stop_zone_scan =
                        std::chrono::high_resolution_clock::now();
                    auto duration_zone_scan =
                        std::chrono::duration_cast<std::chrono::nanoseconds>(
                            stop_zone_scan - start_zone_scan);
...
#ifdef SUBBFS
#ifdef OSMPROFILE
                            auto start_sublevelbf =
                                std::chrono::high_resolution_clock::now();
#endif
                            // bool zoneBFResult =
                            // subfilters[qualifying_zones[i]]->query(str_key);
                            zoneBFResult = subfilters[i]->query(str_key);

#ifdef OSMPROFILE
                            auto stop_sublevelbf =
                                std::chrono::high_resolution_clock::now();
                            auto duration_sublevelbf =
                                std::chrono::duration_cast<
                                    std::chrono::nanoseconds>(stop_sublevelbf -
                                                              start_sublevelbf);
                            sublevel_bf_scan_time +=
                                duration_sublevelbf.count();
#endif
...
// increment #. of sublevel BF positives
...
#ifdef OSMP
                    auto start_unsorted_inter =
                        std::chrono::high_resolution_clock::now();
#endif
                    // if we didn't find the result here, we will have to now do
                    // interpolation search this interpolation search will be
                    // done on each of the sorted blocks after previous_boundary

                    // we initialize the number of blocks that we need to
                    // perform interpolation search on this will always be equal
                    // to number of boundaries we have (because last boundary is
                    // inclusive)
                    int num_interpolation_blocks = subsorted_size;

                    // we perform the interpolation search from the rightmost to
                    // the leftmost block this is because rightmost block will
                    ...
                    while (num_interpolation_blocks > 0) {
                        ...
                        // now that we have defined the boundaries in terms of
                        // zone ids, we need to calculate exact indexes
                        ...
                        // perform interpolation search of the key within this
                        // block
                        ...
                        if (block_interpolation_result) {
                            ...
                            auto stop_unsorted_inter =
                                std::chrono::high_resolution_clock::now();
                            auto duration_unsorted_inter =
                                std::chrono::duration_cast<
                                    std::chrono::nanoseconds>(
                                    stop_unsorted_inter - start_unsorted_inter);
                        ...
                        if (start_b < 0 || num_interpolation_blocks < 0) break;
                    }
            ...
            }  // close the unsorted section scan if gbf approved. If gbf
               // returned false, we directly come here to the sorted section
               // next check for sorted section

            // to have the ability to turn off the zonemaps, let us initialize
            // this result to true always. this way, if we have zonemaps turned
            // off, we will always go inside the following if condition if
            // zonemaps are used, then this flag will be set based on our
            // condition
    ...
    int findFirstGreaterThan(_Key key, int low, int high) {
        ...
        while (low <= high) {
            // int pos = low + ((double)(high - low) / (osmBuffer[high].first -
            // osmBuffer[low].first) * (key - osmBuffer[low].first));
            int pos = (low + high) / 2;
    ...
    void merge(std::vector<std::pair<_Key, _Value>> ar1,
               std::vector<std::pair<_Key, _Value>> ar2, int m, int n,
               std::vector<std::pair<_Key, _Value>> ar3) {
    ...
    std::vector<std::pair<_Key, _Value>> merge(
        std::vector<std::pair<_Key, _Value>> array, int const left,
    ...
    std::vector<std::pair<_Key, _Value>> rangeQuery(_Key low, _Key high) {
        ...
        if (!(high < buf_min) && !(low > buf_max)) {
            ...
            if (!range_sorted) {
                if (atleast_one_subsorted) {
                    // if we have query-sorted components, we sort from the last
                    // sorted component to current
                    last_boundary = subsorted_boundaries[subsorted_size - 1];
                } else {
                    // if we don't have query-sorted components, we sort from
                    // previous_boundary
                    last_boundary = previous_boundary;
                }
            ...
            // since our last part is also going to be sorted, we can do smart
            // search
            int start_index = (num_per_zone * (range_sorted_boundary + 1));
            int end_block = 0;
            if (!(high < osmBuffer[start_index].first) &&
                !(low > osmBuffer[osmBufSize - 1].first)) {
                ...
                if (high < osmBuffer[end_of_block].first) {
                    end = findFirstStrictlyGreaterThan(high, 0, end_of_block) -
                          1;  // first key > high - 1 will be included
                }

                // include all elements from start to end
                result.insert(result.end(), &osmBuffer[start],
                              &osmBuffer[end + 1]);
            }
            ...
            if (atleast_one_subsorted) {
                ...
                // we perform the interpolation search from the rightmost to the
                // leftmost block this is because rightmost block will contain
                ...
                while (num_interpolation_blocks > 0) {
                    ...
                    // now that we have defined the boundaries in terms of zone
                    // ids, we need to calculate exact indexes

                    int end_i =
                        num_per_zone * (right_boundary) + num_per_zone - 1;

                    // lets do some basic sanity checks
                    assert(end_i < osmBufSize);
                    assert(start_i < end_i);

                    // we have start and end boundaries, these will form the
                    // zonemap for this block check if our range falls within
                    // this block if (low >= start_i && low <= end_i)
                    if (!(high < osmBuffer[start_i].first) &&
                        !(low > osmBuffer[end_i].first)) {
                        // interpolation search for first key greater than equal
                        // to low
                        int start = findFirstGreaterThan(low, start_i, end_i);

                        // interpolation search for first key strictly greater
                        // than high
                        int end = end_i;
                        if (high < osmBuffer[end_i].first) {
                            end = findFirstStrictlyGreaterThan(high, start_i,
                                                               end_i) -
                                  1;  // first key > high - 1 will be included
                        }
                        ...
                        // now until old_size is one sorted run and after that
                        // to new_size is next sorted run we can merge it
                        result =
                            merge(result, 0, old_size - 1, result.size() - 1);
                    }
            ...
            // get elements from sorted section until previous boundary
            // (included)
            ...
            if (!(high < sorted_min) && !(low > sorted_max)) {
                ...
                // now until old_size is one sorted run and after that to
                // new_size is next sorted run we can merge it
