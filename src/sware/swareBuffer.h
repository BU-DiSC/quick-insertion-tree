...
template <typename _Key, typename _Value> class OsmBuffer {
  // the actual buffer
...
public:
  ...
  void stlSortOsmBuf() { std::stable_sort(osmBuffer, osmBuffer + osmBufSize); }
  ...
  void sortOsmBuf() {
    ...
    if (out_of_order_elements != 0) {
      ...
      if ((out_of_order_elements < k_limit_1 && max_discrepancy < l_limit_1) ||
          out_of_order_elements < k_limit_2 || max_discrepancy < l_limit_2) {
  ...
  void sortOsmBufNew() {
    ...
    if (out_of_order_elements != 0) {
      ...
      if (atleast_one_subsorted) {
        ...
        auto duration_merge =
            std::chrono::duration_cast<std::chrono::nanoseconds>(stop_merge -
                                                                 start_merge);
        ...
        auto duration_gen =
            std::chrono::duration_cast<std::chrono::nanoseconds>(stop_gen -
                                                                 start_gen);
    ...
    if (element.first >= last_element) {
      ...
      if (current_zone == 0 || last_sorted_zone == -1) {
        last_sorted_zone = -1;
                 ...
                 zones[last_sorted_zone].second) // was second; not as bug
      {
...
// add element to bloom filter
    ...
    auto duration_globalbf =
        std::chrono::duration_cast<std::chrono::nanoseconds>(stop_globalbf -
                                                             start_globalbf);
    ...
    auto duration_sublevel =
        std::chrono::duration_cast<std::chrono::nanoseconds>(stop_sublevel -
                                                             start_sublevel);
    ...
    auto duration_ins = std::chrono::duration_cast<std::chrono::nanoseconds>(
        stop_ins - start_ins);
  ...
  bool query(_Key key) {
    ...
    if (within_buf_range) {
      ...
      if (within_unsorted_section_range) {
        ...
        auto duration_bf = std::chrono::duration_cast<std::chrono::nanoseconds>(
            stop_bf - start_bf);
        ...
        if (result) {
          ...
          if (filled_percent > fill_threshold) {
            ...
            // boundary included
            int start_index = (num_per_zone * (last_boundary + 1));
            int end_index = (num_per_zone * (current_boundary + 1)) -
          ...
          for (int i = current_zone; i >= last_boundary + 1; i--) {
            ...
            if (key >= zones[i].first && key <= zones[i].second) {
              ...
              auto duration_sublevelbf =
                  std::chrono::duration_cast<std::chrono::nanoseconds>(
                      stop_sublevelbf - start_sublevelbf);
...
// increment #. of sublevel BF positives
      ...
      within_sorted_section_range =
          previous_boundary > 0 && (key >= sorted_min && key <= sorted_max);
      ...
      if (within_sorted_section_range) {
        ...
        auto duration_bin =
            std::chrono::duration_cast<std::chrono::nanoseconds>(stop_bin -
                                                                 start_bin);
  ...
  merge(std::vector<std::pair<_Key, _Value>> array, int const left,
