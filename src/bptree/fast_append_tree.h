...
public:
    FastAppendTree(const char *filepath, uint32_t blocks_in_memory, float split_frac = 0.8) :
            super(filepath, blocks_in_memory, split_frac) {}
    ...
    bool insert(const key_type &key, const value_type &value) override {
        if (super::size > 0 && key >= super::tail_min) {
