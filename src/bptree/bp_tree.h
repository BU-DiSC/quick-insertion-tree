...
template <typename key_type, typename value_type> class bp_tree
{
    friend std::ostream &operator<<(std::ostream &os, const bp_tree &tree)
    {
        os << tree.ctr_size << ", " << +tree.ctr_depth << ", " << tree.manager << ", " << tree.ctr_internal << ", "
           << tree.ctr_leaves << ", "
#ifdef REDISTRIBUTE
    ...
    bool leaf_insert(node_t &leaf, const path_t &path, const key_type &key, const value_type &value)
    {
        ...
        if (leaf.info->id == fp_id)
        {
            ...
            else if (lol_prev_size >= IQR_SIZE_THRESH)
            {
                ...
                {
                    split_leaf_pos = outlier_pos; // keep these good values on current lol
                                                  // and do not move
                }
                else
                {
