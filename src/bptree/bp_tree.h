...
class bp_tree {
  ...
  using path_t =
      std::array<key_type, MAX_DEPTH>;  // starts from leaf -> root and empty
                                        // slots at the end for the tree to grow
...
#ifdef FAST_PATH
  void update_paths(uint8_t depth, const key_type &key, uint32_t node_id,
                    uint32_t new_node_id) {
    if (fp_path[depth] == node_id && fp_id != head_id && key <= fp_min) {
      fp_path[depth] = new_node_id;
    }
  }
#endif
  ...
  void internal_insert(const path_t &path, key_type key, uint32_t child_id,
                       uint16_t split_pos) {
    node_t node;
    for (uint8_t i = 1; i < ctr_depth; i++) {
      uint32_t node_id = path[i];
  ...
  void redistribute(const node_t &leaf, uint16_t index, const key_type &key,
    ...
    if (index < items) {
      ...
      std::memcpy(lol_prev.keys + lol_prev_size + index + 1, leaf.keys + index,
                  (items - index) * sizeof(key_type));
      ...
      std::memcpy(lol_prev.values + lol_prev_size + index + 1,
                  leaf.values + index, (items - index) * sizeof(value_type));
  ...
  bool insert_bulk_data_to_leaf(Iterator ibegin, Iterator iend, path_t &path,
                                node_t &leaf, bool is_leaf_root) {
    ...
    // the data will be a vector of pairs, so we need to copy the keys and
    // values separately
  ...
  bool leaf_insert(node_t &leaf, path_t &path, const key_type &key,
                   const value_type &value) {
    manager.mark_dirty(leaf.info->id);
      ...
      if (lol_prev_id == tail_id) {
        ...
        if (lower_pos >
          ...
          uint16_t upper_pos = leaf.value_slot(
              fp_min + upper);  // 0 < split_leaf_pos <= node_t::leaf_capacity
          if (upper_pos <
              SPLIT_LEAF_POS) {  // most of the values are certainly bad
        ...
        uint16_t outlier_pos = leaf.value_slot(
            fp_min +
            max_distance);  // 0 < split_leaf_pos <= node_t::leaf_capacity
                            //                if (key <
                            //                leaf.keys[split_leaf_pos]) {
                            //                    ++split_leaf_pos;
                            //                }
        if (outlier_pos <= SPLIT_LEAF_POS) {
          split_leaf_pos = outlier_pos;  // keep these good values on current
                                         // lol and do not move
...
//                split_leaf_pos = SPLIT_LEAF_POS;
      ...
      bool lol_move = fp_id == head_id ||  // move lol from head
                      (lol_prev_size >= IQR_SIZE_THRESH &&
                       dist(new_leaf.keys[0], fp_min) <
                           IKR::upper_bound(dist(fp_min, lol_prev_min),
                                            lol_prev_size, leaf.info->size));
#endif
      if (lol_move) {
        ctr_iqr++;
    ...
    // for sware, we have different split ratios so we need to update the split
    // position
...
#endif
#ifdef LOL_FAT
    // if the new inserted key goes to lol->next, check if lol->next is not an
    // outlier it might be the case that lol reached the previous outliers.
    if (lol_prev_id !=
            tail_id &&  // lol->prev info exist
                        //            fp_id != head_id && // fp_min is valid
        fp_id != tail_id &&  // fp_max is valid
                             //            leaf.info->id != tail_id && // don't
                             //            go to tail
        fp_max ==
            leaf.keys[0] &&  // leaf is lol->next
                             //            lol_prev_size >= IQR_SIZE_THRESH &&
                             //            lol_size >= IQR_SIZE_THRESH &&  //
                             //            TODO: IQR doesn't have enough values
