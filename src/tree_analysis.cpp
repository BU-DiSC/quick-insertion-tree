...
void insert_worker(tree_t &tree, const std::vector<key_type> &data, Ticket &line, const key_type &offset)
{
    auto idx = line.get();
...
void update_worker(tree_t &tree, const std::vector<key_type> &data, Ticket &line, const key_type &offset)
{
    auto idx = line.get();
    const auto &size = line._size;
    while (idx < size)
...
void query_worker(tree_t &tree, const std::vector<key_type> &data, Ticket &line, const key_type &offset)
{
    size_t idx = line.get();
    const auto &size = line._size;
    while (idx < size)
...
auto work(WorkerFunc worker_func, tree_t &tree, const std::vector<key_type> &data, size_t begin, size_t end,
          uint8_t num_threads, const key_type &offset)
{
    ...
    auto start = std::chrono::high_resolution_clock::now();
    {
        ...
        {
            threads.emplace_back(worker_func, std::ref(tree), std::ref(data), std::ref(line), std::ref(offset));
...
size_t range_queries(tree_t &tree, const std::vector<key_type> &data, size_t num_inserts, size_t range,
                     const key_type &offset, size_t size, std::mt19937 &generator)
{
...
class Workload
{
  ...
  public:
    Workload(tree_t &tree, const Config &conf)
        : tree(tree), conf(conf), offset(0), results(conf.results_csv, std::ofstream::app), generator(conf.seed)
    {
        if (!results)
        {
            std::cerr << "Error: could not open config file " << conf.results_csv << std::endl;
        }
    }
    ...
    void run_preload(const std::vector<key_type> &data, size_t begin, size_t num_load)
    {
        if (num_load > 0)
        {
            std::cout << "Preload (" << num_load << ")\n";
            auto duration = work(insert_worker, tree, data, begin, num_load, conf.num_threads, offset);
            results << ", " << duration.count();
        }
    }

    void run_writes(const std::vector<key_type> &data, size_t begin, size_t raw_writes)
    {
        if (raw_writes > 0)
        {
            std::cout << "Raw write (" << raw_writes << ")\n";
            auto duration = work(insert_worker, tree, data, begin, begin + raw_writes, conf.num_threads, offset);
            results << ", " << duration.count();

            // print timed functions
            std::cout << "t_findLeafSlot: " << tree.t_findLeafSlot << std::endl;
            std::cout << "t_moveExistingInLeaf: " << tree.t_moveExistingInLeaf << std::endl;
            std::cout << "t_insertEntryInLeaf: " << tree.t_insertEntryInLeaf << std::endl;
            std::cout << "t_sortLeaf: " << tree.t_sortLeaf << std::endl;
        }
    }

    void run_reads(const std::vector<key_type> &data, size_t num_inserts, size_t raw_queries)
    {
        if (raw_queries > 0)
        {
            ...
            auto duration = work(query_worker, tree, data, 0, raw_queries, conf.num_threads, offset);
            results << ", " << duration.count();
        }
    }

    void run_mixed(const std::vector<key_type> &data, size_t begin, size_t mixed_writes, size_t mixed_reads)
    {
    ...
    void run_updates(const std::vector<key_type> &data, size_t num_inserts, size_t num_updates)
    {
        if (num_updates > 0)
        {
            ...
            auto duration = work(update_worker, tree, data, 0, num_updates, conf.num_threads, offset);
            results << ", " << duration.count();
        }
    }

    void run_range(const std::vector<key_type> &data, size_t num_inserts, size_t range, size_t size)
    {
        if (range > 0)
        {
            std::cout << "Range (" << range << ")\n";
            auto start = std::chrono::high_resolution_clock::now();
            size_t leaf_accesses = range_queries(tree, data, num_inserts, range, offset, size, generator);
            auto duration = std::chrono::high_resolution_clock::now() - start;
    ...
    void run(const char *name, const std::vector<key_type> &data)
    {
        ...
        const size_t mixed_writes = conf.mixed_writes_perc / 100.0 * num_inserts;
        ...
        results << tree_t::name << ", " << conf.num_threads << ", " << file.filename().c_str() << ", " << offset;
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Running " << tree_t::name << " with " << conf.num_threads << " threads\n";
