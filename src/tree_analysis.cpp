...
auto work(WorkerFunc worker_func, tree_t &tree, const std::vector<key_type> &data, size_t begin, size_t end, uint8_t num_threads, const key_type &offset) {
...
size_t range_queries(tree_t &tree, const std::vector<key_type> &data, size_t num_inserts, size_t range, const key_type &offset, size_t size, std::mt19937 &generator) {
...
public:
  Workload(tree_t &tree, const Config &conf) : tree(tree), conf(conf), offset(0), results(conf.results_csv, std::ofstream::app), generator(conf.seed) {
    if (!results) {
