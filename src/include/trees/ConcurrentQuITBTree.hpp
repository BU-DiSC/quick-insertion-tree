...
public:
  ...
  void find_leaf_exclusive(node_t &node, const key_type &key, key_type &leaf_max) const {
  ...
  bool leaf_insert(node_t &leaf, uint16_t index, const key_type &key, const value_type &value, bool fast) {
  ...
  void split_insert(node_t &leaf, uint16_t index, const path_t &path, const key_type &key, const value_type &value, bool fast) {
    ctr_size++;
    ...
    if (fast) {
      if (leaf.info->id == fp_id) {
        if (fp_prev_id == INVALID_NODE_ID || fp_prev_size < IQR_SIZE_THRESH) {
          fp_move = true;
        } else {
...
public:
  explicit BTree(BlockManager &m) : manager(m), mutexes(m.get_capacity()), root_id(m.allocate()), height(1), life(sqrt(node_t::leaf_capacity)) {
