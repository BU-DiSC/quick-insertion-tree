...
public:
  ...
  explicit BTree(BlockManager &m) : manager(m), root_id(m.allocate()), head_id(m.allocate()), height(1), size(0), life(sqrt(node_t::leaf_capacity)) {
    fp_min = std::numeric_limits<key_type>::min();
    fp_max = std::numeric_limits<key_type>::max();
    fp_id = head_id;

    fp_prev_id = INVALID_NODE_ID;
    fp_prev_min = std::numeric_limits<key_type>::min();
    fp_prev_size = 0;
    fp_size = 0;
  ...
  ~BTree() {
    std::cout << "fast: " << ctr_fast << std::endl;
  }
  ...
  void insert(const key_type &key, const value_type &value) {
    ...
    if (fp_prev_id != INVALID_NODE_ID && fp_max == leaf.keys[0] &&
    ...
    if (fast) {
      if (index < SPLIT_LEAF_POS) {
        node_id_t new_id;
        split_insert(leaf, index, key, value, fp_max, new_id);
        internal_insert(path, fp_max, new_id);
      } else {
        split_insert(leaf, index, key, value, fp_min, fp_id);
        internal_insert(path, fp_min, fp_id);
      }
    } else {
      node_id_t new_id;
      key_type new_key;
...
private:
  ...
  void redistribute(const node_t &leaf, const path_t &path, uint16_t index, const key_type &key, const value_type &value) {
