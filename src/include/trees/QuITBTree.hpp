...
template <typename key_type, typename value_type> class BTree
{
  public:
    ...
    {
        os << tree.ctr_size << ", " << +tree.ctr_depth << ", " << tree.ctr_internal << ", " << tree.ctr_leaves << ", "
           << tree.ctr_redistribute << ", " << tree.ctr_split << ", " << tree.ctr_iqr << ", " << tree.ctr_soft << ", "
           << tree.ctr_hard << ", " << tree.ctr_fp;
        return os;
    }
  ...
  private:
    void create_new_root(const key_type &key, node_id_t node_id)
    {
        ...
        if (root.info->type == LEAF)
        {
            ...
            root.children = reinterpret_cast<node_id_t *>(root.keys + root.internal_capacity);
        }
        manager.mark_dirty(root_id);
        root.info->size = 1;
        root.keys[0] = key;
    ...
    void redistribute(const node_t &leaf, uint16_t index, const key_type &key, const value_type &value)
    {
        ...
        if (index < items)
        {
            ...
            std::memcpy(lol_prev.values + lol_prev_size + index + 1, leaf.values + index,
    ...
    bool leaf_insert(node_t &leaf, const path_t &path, const key_type &key, const value_type &value)
    {
        ...
        // check if leaf is full; is not, insert
        ctr_size++;
        ...
        std::sort(leaf.keys, leaf.keys + leaf.info->size, [&leaf](size_t i, size_t j) {
            ...
            {
                std::swap(leaf.keys[i], leaf.keys[j]);
                std::swap(leaf.values[i], leaf.values[j]);
            }
        });
#endif
        end = std::chrono::high_resolution_clock::now();
