...
class shared_mutex {
   ...
   public:
    ...
    void unlock_shared() { state.fetch_sub(1, std::memory_order_release); }

    void lock() {
        while (true) {
            numeric_type expected = 0;
            if (state.compare_exchange_weak(expected, LOCKED,
                                            std::memory_order_acquire)) {
                break;
            }
#ifdef YIELD
...
class shared_mutex {
   ...
   public:
    void lock_shared() {
        while (true) {
            numeric_type expected =
                active_reads.load(std::memory_order_acquire);
            if (expected != LOCKED &&
                pending_writes.load(std::memory_order_acquire) == 0 &&
                active_reads.compare_exchange_weak(expected, expected + 1,
                                                   std::memory_order_acquire)) {
                break;
            }
#ifdef YIELD
    ...
    void lock() {
        ...
        while (true) {
            numeric_type expected = 0;
            if (active_reads.compare_exchange_weak(expected, LOCKED,
                                                   std::memory_order_acquire)) {
                break;
            }
#ifdef YIELD
    ...
    void unlock() { active_reads.store(0, std::memory_order_release); }
};
}  // namespace srv
...
class shared_mutex {
   ...
   public:
    shared_mutex() { pthread_spin_init(&lk, PTHREAD_PROCESS_PRIVATE); }
...
class shared_mutex {
   ...
   public:
    ~shared_mutex() { pthread_rwlock_destroy(&lk); }

    void lock() { pthread_rwlock_wrlock(&lk); }

    void unlock() { pthread_rwlock_unlock(&lk); }

    void lock_shared() { pthread_rwlock_rdlock(&lk); }

    void unlock_shared() { pthread_rwlock_unlock(&lk); }
};
}  // namespace rwl
...
class shared_mutex {
   ...
   public:
    ...
    void lock_shared() { mtx.lock(); }

    void unlock_shared() { mtx.unlock(); }
};
}  // namespace mtx
